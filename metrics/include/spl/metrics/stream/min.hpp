...
namespace spl::metrics::stream {
     ...
     * auto min_metric = spl::metrics::stream::min<Trade>{timeline};
     *
     * // Insert new element
     ...
     *     min_metric(begin, end);
     * });
     *
     * // Query minimum
     * auto min_price = min_metric();
     * @endcode
     */
    ...
    struct min {
        ...
        /**
         * @brief Construct a streaming minimum metric
         */
        constexpr min() noexcept = default;

        /**
         * @brief Query the current minimum value
         * @return The minimum price in the current window, or error if empty
         * @complexity O(1)
         */
        ...
        constexpr auto operator()(IteratorT begin, IteratorT end) noexcept -> void {
            ...
            // Only decrement count if the removed price matches the current minimum
            for (auto it = begin; it != end; ++it) {
                ...
                // Only touch front if this removal affects current minimum
         ...
         * @complexity O(1) amortized - maintains monotonic increasing property
         */
        constexpr auto operator()(ObjectT const& value) noexcept -> void {
            ...
            // Remove elements from back that are >= new price (they can't be future minimums)
