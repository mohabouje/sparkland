...
static auto verify_against_scan(timeline_type const& timeline, MeanStreamT const& mean_stream, double tolerance = 1e-9) -> void {
    if (timeline.empty()) {
        return;
    }
    ...
    EXPECT_LE(diff, tolerance)
        << "Mean mismatch: stream returned " << result_val
...
TEST(MeanTest, IncreasingSequence) {
    ...
    for (int i = 1; i <= 10; ++i) {
        add(timeline, spl::types::price::from(static_cast<double>(i * 10)),
            static_cast<int64_t>(i) * 1'000'000'000, mean_stream);
    }
...
TEST(MeanTest, LargeNumberOfElements) {
    ...
    for (int i = 0; i < 100; ++i) {
        add(timeline, spl::types::price::from(static_cast<double>(i + 1)),
            static_cast<int64_t>(i) * 1'000'000'000, mean_stream);
    }
...
TEST(MeanTest, AccumulationAccuracy) {
    ...
    auto const result = mean_stream();
    ASSERT_TRUE(result);
    EXPECT_NEAR(static_cast<double>(result.value()), 100.123456789, 1e-6);
