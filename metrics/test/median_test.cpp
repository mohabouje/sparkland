...
static auto verify_against_scan(timeline_type const& timeline, MedianStreamT const& median_stream,
    ...
    EXPECT_LE(diff, tolerance) << "Median mismatch: stream returned " << result_val << ", scan returned "
                               << expected_val << ", difference = " << diff;
}
...
TEST(MedianTest, RemovalUpdatesMedian) {
    ...
    timeline.flush(now, [&](auto begin, auto end) { median_stream(begin, end); });
