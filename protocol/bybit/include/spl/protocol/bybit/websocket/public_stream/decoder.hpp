...
namespace spl::protocol::bybit::websocket::public_stream {
    ...
    struct decoder {
        ...
        [[nodiscard, gnu::hot]] constexpr auto operator()(BufferT&& buffer, HandlerT&& handler) const noexcept
            -> error_type {
            auto const [candidate, header] = tagger_.template hash<'o', 'p'>(buffer);
            auto const view                = buffer.subspan(header);
            switch (hasher::hash(candidate)) {
                case hasher::template hash<spl::protocol::bybit::websocket::public_stream::ping>(): {
                    using object_type  = spl::protocol::bybit::websocket::public_stream::ping;
                    auto const bytes   = err_return(decode<object_type>(view, std::forward<HandlerT>(handler)));
                    auto const decoded = header + bytes;
                    return decoded;
                }
                case hasher::template hash<spl::protocol::bybit::websocket::public_stream::pong>(): {
                    using object_type  = spl::protocol::bybit::websocket::public_stream::pong;
                    auto const bytes   = err_return(decode<object_type>(view, std::forward<HandlerT>(handler)));
                    auto const decoded = header + bytes;
                    return decoded;
                }
                case hasher::template hash<spl::protocol::bybit::websocket::public_stream::subscribe::response>(): {
                    using object_type  = spl::protocol::bybit::websocket::public_stream::subscribe::response;
                    auto const bytes   = err_return(decode<object_type>(view, std::forward<HandlerT>(handler)));
                    auto const decoded = header + bytes;
                    return decoded;
                }
