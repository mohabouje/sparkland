...
#include "spl/protocol/coinbase/websocket/public_stream/subscribe/request.hpp"
#include "spl/protocol/coinbase/websocket/public_stream/unsubscribe/request.hpp"
...
namespace spl::protocol::coinbase::websocket::public_stream {
    ...
    struct encoder {
        ...
        using value_type   = std::variant<spl::protocol::coinbase::websocket::public_stream::subscribe::request,
                                          coinbase::websocket::public_stream::unsubscribe::request>;
        using tagger_type  = TaggerT<value_type>;
        using error_type   = typename encoder_type::error_type;

        struct hasher {
            constexpr static auto mapper = frozen::unordered_map<frozen::string, std::size_t, 2>{
                {spl::reflect::identifier<
                     spl::protocol::coinbase::websocket::public_stream::subscribe::request>::unique_id,
                 0                                                                                                       },
                {spl::reflect::identifier<
                     spl::protocol::coinbase::websocket::public_stream::unsubscribe::request>::unique_id,
                 1                                                                                                       },
            };
        ...
        [[nodiscard, gnu::hot]] constexpr auto operator()(ObjectT&& object, std::span<char> buffer) noexcept
            ...
            auto const header  = err_return(tagger_.template header<'t', 'y', 'p', 'e'>(tag, buffer));
            ...
            auto const footer  = err_return(tagger_.template footer<'t', 'y', 'p', 'e'>(tag, overall));
